...
os.system("rm -f *dat && ./cunfft_adjoint s %s %s %s"%(sys.argv[1], sys.argv[2], sys.argv[3]))
...
orig = np.loadtxt("original_signal.dat", dtype=dt_real)
...
gridded_data = np.loadtxt('gridded_data.dat', dtype=dt_real)
#gridded_filter = np.loadtxt('unequally_spaced_gridded_filter.dat', dtype=dt_complex)

fft_adjoint = np.loadtxt('adjoint_nfft.dat', dtype=dt_complex)
...
ax.plot(gridded_data['x'] * T / len(gridded_data) - 0.5, gridded_data['y'], 'k:', label="gridded")
...
kmax = np.argmax(mod(fft_raw_data)[1:])
fmax = fft_raw_data['index'][kmax] / T
print fmax / freq

kmax_should_be =  freq * T / (2 * np.pi)
print kmax, kmax_should_be

ax2.plot(fft_raw_data['index'] / T, mod(fft_raw_data), 'k:', label="FFT(gridded data) [raw]" )
...
ax2.plot(fft_adjoint['index'] / T, mod(fft_adjoint), color='k', lw=2, label="FFT adjoint" , alpha = 0.6)
#ax2.plot(fft_adjoint_equal['index'] * 2 * np.pi / T, mod(fft_adjoint_equal), color='r', lw=2, label="EQSP FFT adjoint" , alpha = 0.6 )

ax2.set_ylim(0.1 * np.median(mod(fft_adjoint)), 1.3*max(mod(fft_raw_data)))
ax2.legend(loc='best')
